/// Provides constants for (some of) the well-known character set collations
/// found in various versions of MySQL and compatible implementations.
///
/// Like Swift itself, this package considers UTF-8 its native character set.
/// As such, no effort is made to support the use of non-UTF-8 character sets;
/// clients who perform a manual `SET NAMES` query are likely to quicky encounter protocol errors or, worse, data corruption.
///
/// In particular, this package requires the use of what MySQL calls the `utf8mb4` character set,
/// which corresponds to the canonical UTF-8 encoding defined by BCP 18.
///
/// The `utf8mb4` character offers many different collations (text normalization and comparison rules),
/// some of which are only available in newer MySQL versions, while others are only available in recent versions of MariaDB.
/// The collations of greatest interest for language-nonspecific usage are given by the following table:
///
...
///
/// Unfortunately, the UCA 9.0-based collations are available only in MySQL 8, Percona 8, and Aurora 3,
/// while the UCA 14.0-based collations are available only in MariaDB 10.10 and later.
/// To ensure the best collation available is used whenever possible,
/// this package uses the following heuristic to choose a collation:
///
/// 1. If `CLIENT_MYSQL` is absent and version `>= 10.10` (new MariaDB):
///
///    - Use `utf8mb4_uca1400_ai_ci`
/// 2. If `CLIENT_MYSQL` and `CLIENT_QUERY_ATTRIBUTES` are present (new MySQL):
///
...
struct MySQLCollation {
    ...
    /// are exposed via the MariaDB-specific table `information_schema.collation_character_set_applicability`.
    /// See [MariaDB Server Ticket MDEV-27009](https://jira.mariadb.org/browse/MDEV-27009) for more details.
    let id: UInt16
    ...
    /// If the given ID corresponds to a "well-known" collation (see above), returns that collection.
    /// Otherwise returns a synthesized collation with empty names.
    static func lookup(byId id: UInt16) -> Self {
        switch id {
        case Self.binary.id: Self.binary
        case Self.utf8Binary.id: Self.utf8Binary
        case Self.utf8GeneralCI.id: Self.utf8GeneralCI
        case Self.utf8Unciode4CI.id: Self.utf8Unciode4CI
        case Self.utf8Unicode52CI.id: Self.utf8Unicode52CI
        case Self.utf8Unicode9AICI.id: Self.utf8Unicode9AICI
        case Self.utf8Unicode9ASCI.id: Self.utf8Unicode9ASCI
        case Self.utf8Unicode9ASCS.id: Self.utf8Unicode9ASCS
        case Self.utf8Unicode14AICI.id: Self.utf8Unicode14AICI
        case Self.utf8Unicode14AICS.id: Self.utf8Unicode14AICS
        case Self.utf8Unicode14ASCI.id: Self.utf8Unicode14ASCI
        case Self.utf8Unicode14ASCS.id: Self.utf8Unicode14ASCS
        default: .init(name: "", characterSetName: "", id: id)
        }
    }
}
