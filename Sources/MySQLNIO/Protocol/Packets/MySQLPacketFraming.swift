...
final class MySQLRawPacketCodec: NIOSingleStepByteToMessageDecoder, NonThrowingMessageToByteEncoder {
    ...
    struct RawPacketFrame: RawRepresentable {
        let length: UInt24
    ...
    enum SequencingState: Equatable {
        /// Initial state
        ///
    ...
    enum FragmentationState: Equatable {
    ...
    init(sequenceCounter: UInt8, sequencingState: SequencingState, fragmentationState: FragmentationState) {
    ...
    func encode(data: ByteBuffer, out: inout ByteBuffer) {
        ...
        if data.readableBytes < Int(UInt24.max) + MemoryLayout<RawPacketFrame>.size {
            ...
            out.setInteger(
                RawPacketFrame(length: out.readableBytes - 4, sequenceId: self.nextSequenceID()).rawValue,
                at: out.readerIndex,
                endianness: .little
            )
        } else {
            ...
            // N.B.: We already checked the length explicitly
            out = remainingData.readSlice(length: Int(UInt24.max) + MemoryLayout<RawPacketFrame>.size)!
            // fill in the first fragment's info
            out.setInteger(RawPacketFrame(length: UInt24.max, sequenceId: self.nextSequenceID()).rawValue, at: out.readerIndex, endianness: .little)
            // additional frames, including terminator, minus the first one
            out.reserveCapacity(minimumWritableBytes: remainingData.readableBytes + numFullFragments * 4)
            while remainingData.readableBytes >= UInt24.max {
                out.writeInteger(RawPacketFrame(length: UInt24.max, sequenceId: self.nextSequenceID()).rawValue, endianness: .little)
                out.writeImmutableBuffer(remainingData.readSlice(length: Int(UInt24.max))!)
            }
            // even if there are exactly zero bytes left, we want to send that
            out.writeInteger(RawPacketFrame(length: remainingData.readableBytes, sequenceId: self.nextSequenceID()).rawValue, endianness: .little)
            out.writeImmutableBuffer(remainingData.slice())
        }
    }

    // See `NIOSingleStepByteToMessageDecoder.decode(buffer:)`.
    func decode(buffer: inout ByteBuffer) throws(Error) -> ByteBuffer? {
        ...
        guard let frameWord = buffer.getInteger(at: buffer.readerIndex, endianness: .little, as: UInt32.self),
        ...
        guard
            frame.sequenceId == self.sequenceCounter
                || (self.sequencingState == .resetOnOutgoing && self.fragmentationState == .initial && frame.sequenceId == 1
                    && frame.length > 9  // Min ERR_Packet length
                    && buffer.readableBytesView.first == 0xff  // ERR_Packet marker
                        )
        else {
        ...
        if frame.length == .max {
            ...
            case .partiallyReceived(var partialBuffer):
                ...
                // we want to advance the reader index on the input buffer
                partialBuffer.writeImmutableBuffer(buffer.readSlice(length: Int(UInt24.max))!)
        ...
        if self.sequencingState == .waitingForInitialOk, frame.length >= 7,  // Min OK_Packet length
            ongoingBuffer.readableBytesView.first == 0x00  // OK_Packet marker
        {
    ...
    func decodeLast(buffer: inout ByteBuffer, seenEOF _: Bool) throws(Error) -> ByteBuffer? {
