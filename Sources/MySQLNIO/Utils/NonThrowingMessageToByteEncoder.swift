...
protocol NonThrowingMessageToByteEncoder {
...
final class MessageToByteProcessor<Encoder: MessageToByteEncoder> {
    @usableFromInline
    enum State: Hashable {
        case flushable
...
final class NonThrowingMessageToByteProcessor<Encoder: NonThrowingMessageToByteEncoder> {
    @usableFromInline internal private(set) var encoder: Encoder
    @usableFromInline internal private(set) var buffer: ByteBuffer
...
final class MessageByteTranscodingProcessor<Codec: NIOSingleStepByteToMessageDecoder & MessageToByteEncoder> {
...
final class NonThrowingMessageByteTranscodingProcessor<Codec: NIOSingleStepByteToMessageDecoder & NonThrowingMessageToByteEncoder> {
    @usableFromInline internal private(set) var codec: Codec
    @usableFromInline internal let decodeProcessor: NIOSingleStepByteToMessageProcessor<Codec>
    ...
    @inlinable func process(buffer: ByteBuffer, _ cb: (Codec.InboundOut) throws -> Void) throws {
        try self.decodeProcessor.process(buffer: buffer, cb)
    }
    @inlinable func finishProcessing(seenEOF: Bool, _ cb: (Codec.InboundOut) throws -> Void) throws {
        try self.decodeProcessor.finishProcessing(seenEOF: seenEOF, cb)
    }
