...
import Foundation
import Crypto
#if canImport(MiniRSACrypt)
#warning("Replace with SwiftCrypto version ASAP")
import MiniRSACrypt
#else
...
extension MySQLBuiltinAuthHandlers {
    /// Implements MySQL's `caching_sha256_password` plugin
    ///
    ...
    struct CachingSHA256: MySQLBuiltinAuthHandler {
        static var pluginName: String { "caching_sha256_password" }
        ...
        mutating func processData(_ data: ByteBuffer, configuration: MySQLChannel.Configuration, connectionIsSecure: Bool) throws -> ByteBuffer? {
            switch self.state {
                case .awaitingNonce:
                    guard data.readableBytes == 20 else { throw MySQLChannel.Error.protocolViolation } // nonce must be 20 bytes exactly, even if we won't use it
                ...
                case .requestedServerKey(nonce: let nonce):
                    ...
                    guard let pemString = data.getString(at: data.readerIndex, length: data.readableBytes) else { throw MySQLChannel.Error.protocolViolation }
        ...
        private enum State {
            ...
            /// -|:-:|-
            /// ||_initial_|||
            /// `awaitingNonce`|→|``passwordSent``
            /// `awaitingNonce`|→|``sentFastAuth(nonce:)``
            /// `awaitingNonce`|→|``failure(error:)``
            /// ||||
            case awaitingNonce
            ...
            /// ||||
            /// -|:-:|-
            /// `sentFastAuth`|→|``passwordSent``
            /// `sentFastAuth`|→|``requestedServerKey(nonce:)``
            /// `sentFastAuth`|→|``failure(error:)``
            /// ||||
            ...
            /// `requestedServerKey`|→|``failure(error:)``
            /// ||||
