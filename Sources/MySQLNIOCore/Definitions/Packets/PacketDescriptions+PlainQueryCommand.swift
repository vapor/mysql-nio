...
extension MySQLPacketDescriptions {
    ...
    struct PlainQueryCommand {
        ...
        init?(from packet: ByteBuffer, activeCapabilities capabilities: MySQLCapabilities) throws {
            var packet = packet
            guard try packet.mysql_readMarker(matching: Self.commandByte) else { return nil }
            ...
            guard let sql = packet.readString(length: packet.readableBytes) else { throw MySQLChannel.Error.protocolViolation }
