...
final class MySQLNIOTests: XCTestCase {
    ...
    func testCryptoUtils() throws {
        let digestSha1 = sha1(ByteBuffer(string: "test 1"), ByteBuffer(string: "test 2"))
        let digestSha256 = sha256(ByteBuffer(string: "test 1"), ByteBuffer(string: "test 2"))
        let digestsXor = xor(digestSha1, digestSha256.getSlice(at: 0, length: digestSha1.readableBytes)!)
        let digestsPat = xor_pattern(digestSha256, digestSha1)
        
        XCTAssertEqual(digestSha1.readableBytesView, [
            0x03, 0x83, 0xf2, 0x6b, 0x7b, 0xe9, 0xfe, 0xcf, 0x63, 0xd1,
            0x8c, 0xa2, 0x33, 0xb3, 0x8e, 0x51, 0x16, 0xfc, 0x54, 0xa2,
        ])
        XCTAssertEqual(digestSha256.readableBytesView, [
            0x80, 0xa6, 0xc8, 0xee, 0x85, 0xe8, 0x27, 0xc6, 0xbf, 0xf2, 0xd0, 0x51, 0x21, 0x6a, 0xaf, 0x42,
            0x4c, 0x9e, 0x26, 0x39, 0x4f, 0x09, 0x24, 0xb6, 0x6d, 0x46, 0xa5, 0x43, 0x17, 0x54, 0x79, 0x75,
        ])
        XCTAssertEqual(digestsXor.readableBytesView, [
            0x83, 0x25, 0xc1, 0x85, 0xfe, 0x01, 0xd9, 0x09, 0xdc, 0x23,
            0x5c, 0xf3, 0x12, 0xd9, 0x21, 0x13, 0x5a, 0x62, 0x72, 0x9b,
        ])
        XCTAssertEqual(digestsPat.readableBytesView, [
            0x83, 0x25, 0xc1, 0x85, 0xfe, 0x01, 0xd9, 0x09, 0xdc, 0x23, 0x5c, 0xf3, 0x12, 0xd9, 0x21, 0x13,
            0x5a, 0x62, 0x72, 0x9b, 0x43, 0x8a, 0xd6, 0xdd, 0x16, 0xaf, 0x5b, 0x8c, 0x74, 0x85, 0xf5, 0xd7,
        ])
    }
